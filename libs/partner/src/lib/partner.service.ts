...
export class PartnerService {
  ...
  async getAllPartnerSupplier(take=10,skip=0):Promise<Partner[]|Message> {
...
//   async addNewSupplier(supplier):Promise<Supplier|Message>{
  async addNewSupplier(supplier){
    const {name,phone,latitude,longitude}:{name:string,phone:number,latitude:string,longitude:string} = supplier;
    try{
      return await this.prisma.partner.update(
        {
          data:{
